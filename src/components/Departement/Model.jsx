...
export default function KeepMountedModal({id,open,handleClose}) {
    ...
    const { data, error, isLoading } = useQuery({
        queryKey: ["editDepartement", { id }],
        queryFn: async () => axiosClient.get(`/departement/${id}`).then(({ data }) => data.data)

    });

    const updateDepartementMutation = useMutation({
        mutationFn: async (values) => await axiosClient.put(`/departement/${id}`, values),
        onSuccess: (data) => {
            console.log(data)
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            ...
            updateDepartementMutation.mutate(values);
        }
    });
